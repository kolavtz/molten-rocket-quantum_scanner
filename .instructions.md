...
- for authentication and RBAC, implement a secure baseline that can be extended with real user management in the future, but do not hardcode any credentials or roles in source. Use environment variables and configuration to control access and behavior, and ensure that the system can operate securely even in a development environment without seeded users.
